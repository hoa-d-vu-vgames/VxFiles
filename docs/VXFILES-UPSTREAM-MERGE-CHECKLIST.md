...
- A dialog opens with its title and buttons but no body: check that `DynamicDialog.xaml` still uses `Visibility` rather than deferred `x:Load`. Files `v4.2.2` adopted this fix upstream.
