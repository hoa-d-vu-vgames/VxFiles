...
/// <summary>
/// Covers a whole Automation Package: its canonical manifest, every file it contains, the app-local Python
/// runner, and the configured external executables. Any change renews package trust for all of its actions.
/// </summary>
internal static class AutomationTrustFingerprint
{
	...
	private static async ValueTask<string> FingerprintTreeAsync(string rootPath, bool canonicalizeManifest)
	{
		...
		foreach (var path in Directory.EnumerateFileSystemEntries(root, "*", SearchOption.AllDirectories).Order(StringComparer.Ordinal))
		{
			...
			if ((attributes & FileAttributes.ReparsePoint) != 0)
				throw new InvalidOperationException($"Reparse-backed package or runtime entry is forbidden: {path}");
	...
	/// <summary>
	/// Reformatting a manifest must not renew trust; changing a value must.
	/// </summary>
	...
	/// <summary>
	/// Length-prefixes each record so that no combination of names and contents can collide.
	/// </summary>
