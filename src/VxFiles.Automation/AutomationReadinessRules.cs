...
{
	/// <summary>
	/// Returns the display name of every external tool this action needs and cannot use.
	/// </summary>
	/// <remarks>
	/// The host owns the localized sentence. Keeping only the manifest's display name here lets the headless module
	/// identify the program without publishing UI text.
	/// </remarks>
		...
		AutomationPackageState state)
	{
		...
		foreach (var toolId in action.ExternalToolIds)
		{
			...
			{
				diagnostics.Add(definition.DisplayName);
				continue;
			}
