	...
	public sealed class GoogleDriveCloudDetector : AbstractCloudDetector
	{
		...
		protected override async IAsyncEnumerable<ICloudProvider> GetProviders()
		{
			...
			await StorageFile.GetFileFromPathAsync(Path.Combine(appDataPath, @"Google\DriveFS\root_preference_sqlite.db")).AsTask()
				.AndThen(async c => await c.CopyAsync(await VxFilesEnvironment.GetTemporaryFolderAsync(), "google_drive.db", NameCollisionOption.ReplaceExisting));
			...
			await FilesystemTasks.Wrap(() => StorageFile.GetFileFromPathAsync(Path.Combine(appDataPath, @"Google\DriveFS\root_preference_sqlite.db-wal")).AsTask()
				.AndThen(async c => await c.CopyAsync(await VxFilesEnvironment.GetTemporaryFolderAsync(), "google_drive.db-wal", NameCollisionOption.ReplaceExisting)));

			var syncDbPath = Path.Combine(VxFilesEnvironment.TemporaryDataPath, "google_drive.db");
