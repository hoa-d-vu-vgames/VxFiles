...
namespace Files.App.Helpers
{
	...
	public static class ActiveSessionTracker
	{
		...
		public static void OnActivationChanged(bool isActive)
		{
			...
			var totalTime = VxFilesEnvironment.GetState(ActiveTimeKey, 0d);
			var totalStretches = VxFilesEnvironment.GetState(StretchCountKey, 0);
			VxFilesEnvironment.SetState(ActiveTimeKey, totalTime + elapsed.TotalSeconds);
			VxFilesEnvironment.SetState(StretchCountKey, totalStretches + 1);
		}
		...
		public static void ReportPersistedTime()
		{
			...
			var activeTimeSeconds = VxFilesEnvironment.GetState(ActiveTimeKey, 0d);
			if (activeTimeSeconds <= 0d)
				return;
			...
			var stretchCount = VxFilesEnvironment.GetState(StretchCountKey, 0);
			if (stretchCount > 0)
				transaction.SetMeasurement("active_stretches", stretchCount);
			transaction.Finish();

			VxFilesEnvironment.RemoveState(ActiveTimeKey);
			VxFilesEnvironment.RemoveState(StretchCountKey);
		}
	}
}
